...
class IdempotentLoss(nn.Module):
    ...
    def forward(self, input_data, output_data, training_model):
        ...
        output_data = output_data.view(1, 1, -1)
        ...
        z = z.squeeze()
        _, fz, _, _ = training_model(z)
        f_z = fz.detach()
        _, ff_z, _, _ = training_model(f_z)
        _, f_fz, _, _ = self.training_model_copy(fz)
    ...
    def get_freq_means_and_stds(self, x):
        ...
        # real_std = freq.real.std(dim=0).clamp(min=1e-10)
        real_var = torch.var(freq.real, dim=0, unbiased=False)
        real_std = torch.sqrt(real_var + 1e-10)

        imag_mean = freq.imag.mean(dim=0)
        # imag_std = freq.imag.std(dim=0).clamp(min=1e-10)
        imag_var = torch.var(freq.imag, dim=0, unbiased=False)
        imag_std = torch.sqrt(imag_var + 1e-10)
